...
"Error, there is no planet there","615":"Error, no hay suficiente espacio de carga","616":"Multi-alarm","617":"Admin or GM"}},'en':{save:"Save",cancel:"Cancel",ai:"Espionage",dR:"Recycle",gr:
"No available recyclers",cD:"Easy Ally Communication",hl:"The message has been sent",send:"Send",dM:"Galaxy",cW:"Fleet",cA:"General",dO:"Buildings",kP:"Messages",fm:"Info",cZ:"Search updated every",
cr:"Never",aF:"days",cn:"Extended details on construction",cJ:"Shorten values",du:"Show highlighted debris",cQ:"Highlight debris (medium)",dz:"Highlight debris (high)",aU:"Show rank on name player",
cs:"Highlight friends and allies",db:"Highlight link coordinates",cC:"Spy moons with a click",cY:"Recycle with a click",cP:"Display the resources in transit",ht:"Ships in FleetII/FleetIII",hG:
"Shortcuts in FleetII",hn:"Shortcut",cb:"Deuterium consumption",au:"Unknown",cV:"Available in",dW:"Produce maximum number",dA:"A new version of plugin XiTools.",da:"Update",cg:"version",cG:
"Error, Medium debris:\nRequired.\nNumber.",bx:"Error, High debris:\nRequired.\nNumber.\nGreater than Low debris.",cF:"Planets",dr:"M",cB:"HW",cE:"Show detail information",ce:"Highscore",bG:{0:
"Score: %t (%p pts.)",1:"Economy: %t (%p pts.)",2:"Research: %t (%p pts.)",3:"Military: %t (%p pts.)",4:"Military lost: %t (%p pts.)",5:"Military built: %t (%p pts.)",6:
...
"Admin or GM","618":"Attack ban until 01.01.1970 01:00:00"}},'ru':{save:"Сохранить",cancel:"Отмена",ai:"Шпионаж",dR:"Переработать",gr:"Нет доступных переработчиков",cD:"Отправка сообщения альянсу",hl:
"Сообщение было отправлено",send:"Отправить",dM:"Галактика",cW:"Флоты",cA:"Остальное",dO:"зданий",kP:"сообщения",fm:"Info",cZ:"Поиск обновляется каждые",cr:"никогда",aF:"дней",cn:
"Расширенные сведения о строительстве",cJ:"Сократить значения",du:"Включить подсветку обломков",cQ:"Подсветка обломков (средне)",dz:"Подсветка обломков (много)",aU:"Показывать рейтинг игрока",cs:
"Подсветка друзей и альянсов",db:"Подсветка координат",cC:"шпион Луны с помощью щелчка",cY:"Переработать одним щелчком",cP:"Показывать ресурсы в полете",ht:"Доставка FleetII/FleetIII",hG:
"Клавиши быстрого доступа в FleetII",hn:"контекстного",cb:"Дейтерий потребления",au:"неизвестный",cV:"Доступно в",dW:"Produce maximum number",dA:"Новая версия плагина XiTools.",da:"обновление",cg:
"версия",cG:"Ошибка, Средне количество обломков:\nТребуется ввести число побольше.",bx:"Ошибка, Большое количество обломков:\nТребуется ввести число больше, .\nчем указано для средних обломков.",cF:
"Планеты",dr:"M",cB:"ГП",cE:"Больше+",ce:"Статистика",bG:{0:"По очкам: %t (%p очков)",1:"Экономика: %t (%p очков)",2:"Исследования: %t (%p очков)",3:"Боевой рейтинг: %t (%p очков)",4:
"Боевых очков потеряно: %t (%p очков)",5:"Боевых очков набрано: %t (%p очков)",6:"Боевых очков уничтожено: %t (%p очков)",7:"Очки чести: %t (%p очков)",8:"единиц"},eD:"Подсветка планет",cU:
"Нет подсвечиваемых планет.",mN:"Account activity",nv:"Activity on",nm:"Coords",mI:"Planet",lE:"Moon",nk:"Time",nb:"out of deuterium",mE:"destroyed planet",cq:{0:"Металл",1:"Кристалл",2:"Дейтерий"},
bZ:"Всего",fk:"Ожидается ресурсов на планете",jd:"Простая транспортировка",ik:"Транспорт",jo:"все",ja:"потерявшийся",iq:"Reset all data",ap:"Info Production",ev:
...
"Defense production",bD:"Конвертер боевых докладов",aq:"ACS Debris Distribute",jX:"Universe speed",kk:"Fleet to debris",kN:"Defense to debris",lbleblselbytype:"Selection by type",jQ:
...
window.location.search;var mZ={};bu.replace(new RegExp("([^?=&]+)(=([^&]*))?","g"),function($0,$1,$2,$3){mZ[$1]=$3;});return mZ;};function XiTClass(call){this.version="3.0.6.1";this.fz=
...
.dialog('close');}});this.fh=(function(){var nq=
...
this.eA();}}};function hx(){var dU=function(){var step=0;var aJ=0;var eh=0;var fr=0;var kt=0;var kv=0;var aR=0;var df=0;var aH=1;this.data;var ay=this;var fL=function(){aJ=ay.data.aI;aR++;var fu=
document.getElementById("missileValue").textContent;var type=0;if(ay.data.mission==6){kv-=aJ;type=1;}else if(ay.data.mission==8){kt-=aJ;type=2;}var response={message:fp.bB["600_"+type],type:type,
slots:aR,probes:kv,km:kt,missiles:fu,shipsSent:aJ,coordinates:{galaxy:ay.data.galaxy,system:ay.data.system,position:ay.data.position},planetType:ay.data.type,success:true};displayMiniFleetMessage(
response);};var aj=function(code){if(!code){code="601";}var response={message:fp.bB[code],coordinates:{galaxy:ay.data.galaxy,system:ay.data.system,position:ay.data.position},success:false};
displayMiniFleetMessage(response);};this.ee=function(){return(aH==1);};this.reset=function(){aH=1;step=0;aJ=0;eh=0;fr=0;delete this.data;};this.cw=function(html){if(aH==1)return;step++;var page;
var method="POST";var F={};switch(step){case 1:page='/game/index.php?page=fleet1';method="GET";break;case 2:if($(html).find('form[action$="fleet2"]').length<1){aj();this.reset();return;}var eb=$(html)
.find("#slots span.advice").eq(0).text();/(\d*)\/(\d*)/.exec(eb);aR=parseInt(RegExp.$1);df=parseInt(RegExp.$2);kt=parseInt($(html).find("#button209 span.level").eq(0).text().replace(/\D/g,""));if(
isNaN(kt))kt=0;kv=parseInt($(html).find("#button210 span.level").eq(0).text().replace(/\D/g,""));if(isNaN(kv))kv=0;$("#slotUsed").html(tsdpkt(aR));setShips("probeValue",tsdpkt(kv));setShips(
"recyclerValue",tsdpkt(kt));if((this.data.mission==6&&kv==0)||(this.data.mission==8&&kt==0)){aj("611");this.reset();return;}else if(aR==df){aj("612");this.reset();return;}var form=$(html).find(
"form[name='shipsChosen']");page=form.attr("action");F[this.data.dJ]=this.data.aI;break;case 3:if($(html).find('form[action$="fleet3"]').length<1){aj();this.reset();return;}var form=$(html).find(
"form[name='details']");page=form.attr("action");F[this.data.dJ]=this.data.aI;F.galaxy=this.data.galaxy;F.system=this.data.system;F.position=this.data.position;F.type=this.data.type;F.iC=this.data.iC;
F.union=this.data.union;F.acsValues=this.data.acsValues;break;case 4:if($(html).find('form[action$="movement"]').length<1){aj();this.reset();return;}var iJ=parseInt($(html).find("#consumption").text()
.replace(/\D/g,""));var ca=parseInt($(html).find("#resources_deuterium").text().replace(/\D/g,""));if(iJ>ca){aj("613");this.reset();return;}var fn=parseInt($(html).find("#maxresources").text()
.replace(/[^0-9-]/g,""));if(fn<0){aj("615");this.reset();return;}var form=$(html).find("form[name='sendForm']");page=form.attr("action")+'&ajax=1';form.find("input,select").each(function(){if(
this.name)F[this.name]=this.value;});F.mission=this.data.mission;break;default:fL();this.reset();return;break;}$.ajax({dataType:"html",url:page,data:F,type:method,timeout:5000,success:function(html){
ay.cw($.parseHTML(html));},error:function(xhr,status,exception){addToTable(exception,"error");ay.reset();}});};this.start=function(){if(typeof this.data!='undefined'){aH=0;this.cw();}}};
...
" div.player-more").html(hw);$("div#player"+k).each(function(){Tipped.refresh($(this)[0]);});}});});XiTClass.prototype.gJ={};XiTClass.prototype.ai=(function(er,en,ec,aX){var di=new dU();if(di.ee()){
di.data={mission:6,type:3,galaxy:er,system:en,position:ec,dJ:'am210',aI:aX,iC:10,union:0,acsValues:'-'};di.start();}return false;});$(document).ajaxSuccess(function(e,xhr,settings){if(
...
"tr.row:nth-child("+iQ+")").css("border","1px solid red").css("margin","-1px");}}}if(XiT.V.eblspy.aT()&&XiT.data.k.commander=="0"){eP.find('div[id*="moon"]:not(.XiT-done)').each(function(){var jm=$(
this);jm.addClass("XiT-done");var eL=jm.attr('id').replace("moon","");var kO=eL.bn();var kT=$.trim(eP.find('tr:nth-child('+kO+') .playername span[class*="status_abbr_"]:first').html());var kv=XiT.eo(
$.trim(eP.find("#probeValue").html()));if(XiT.fG!=kT&&kv>0){eP.find("#moon"+eL+" .ListLinks").append('<li><a href="javascript:void(0);" onClick="XiT.ai('+XiT.eN()+','+XiT.ft()+','+eL+','+
spionageAmount+')">'+fp.ai+'</a></li>');}});}if(XiT.V.eblhld.aT()||XiT.V.eblrecy.aT()){eP.find('div[id*="debris"]:not(.XiT-done)').each(function(){var ed=$(this);ed.addClass("XiT-done");var eL=
ed.attr('id').replace("debris","");var gI=ed.find(".ListLinks li:first").html();var bJ=parseInt(gI.substring(gI.lastIndexOf(" "),gI.length).replace(/\./g,""));var ga=ed.find(
