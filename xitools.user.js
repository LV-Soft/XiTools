...
// @nocompat
// @noframes
// @name			XiTools III for OGame.
// @namespace		XiTools III for OGame.
// @description		XiTools III to improve the user experience of OGame 6.0.6
// @version			3.1.1
...
// ==/UserScript==
function gU(){XiTClass.prototype.mA=(function(){});XiTClass.prototype.kX=false;XiTClass.prototype.jM=0;XiTClass.prototype.kM=0;XiTClass.prototype.iB=0;XiTClass.prototype.nj=0;
XiTClass.prototype.Message=(function(kY){if(!this.V.eblspioplunder.aT()&& !this.V.eblspiodf.aT()&& !this.V.eblspiosim.aT())return false;function getData(kL){var i;var l;var h;var gs;var kx;var time;
var e=kL.find("ul.detail_list li.detail_list_el");var r=kL.find("ul.detail_list li.resource_list_el");if(!e){return null;}var gv={fE:kL.find('.detail_msg_ctn .detail_txt .section_title:last'),data:{},
bJ:r.eq(0).attr("title").replace(/\D/g,'').bn(),kr:r.eq(1).attr("title").replace(/\D/g,'').bn(),ca:r.eq(2).attr("title").replace(/\D/g,'').bn(),bK:r.eq(3).attr("title").replace(/\D/g,'').bn(),
position:[]};l=e.length;for(i=0;i<l;i++){var key=$.trim(e.eq(i).find('span:first').text());var val=e.eq(i).find('span:last').text().replace(/\D/g,'');gv.data[XiT.jW(key)]=val;}var pos=kL.find(
'.msg_actions .icon_nf.icon_attack').parent().attr('href').match(lu);if(pos&&pos.length===5){for(i=1;i<5;i++){gv.position.push(pos[i].bn());}}else{return null;}h=kL.find('.msg_title a.txt_link');if(!
h|| !h.length){return null;}gs=h.eq(0).text().match(/([^\[]*) \[/);if(gs&&gs.length){gv.lk=$.trim(gs[1]);}else{return null;}gv.kx=$.trim(kL.find(".detail_msg_ctn span[class*='status_abbr']").eq(0)
.text());if(kL.find(".detail_msg_ctn span[class*='inactive']")[0]){gv.la=[0.5];}else{gv.la=(kL.find(".detail_msg_ctn span[class*='rank_bandit']")[0])?[1]:(kL.find(
".detail_msg_ctn span[class*='status_abbr_honorableTarget']")[0])?[0.75]:[0.5];}return gv;};var t=$(".detail_msg");var kZ=[];var i;var l=t?t.length:0;var gv;var lu=
/&galaxy=(\d+)&system=(\d+)&position=(\d+)&type=(\d+)&/;var lA=new RegExp("\\(.+\\s\'(.+)\\'\\)");var kA=new RegExp("(\\d{2}).(\\d{2}).(\\d{4})\\s(.+)");for(i=0;i<l;i++){gv=getData(t.eq(i));if(gv){
kZ.push(gv);}}var lw=this.V.eblspioplunder.aT();var ly=this.V.eblspiodf.aT();var jV=this.V.eblspiosim.aT();var kw=(this.V.deftodebris=="1")?(1-this.V.defrepfactor):0;var kI=this.V.debrisfactor;var mq=
document.getElementsByName('ogame-language')[0].content;var mD=this.V.eblosim.aT();var ma=this.V.eblwebsim.aT();var mP=this.V.ebldragosim.aT();var p=this.cj.split(":");for(var key in kZ){var gv=
kZ[key];var fQ="";var percent=0;var m=0;var c=0;var jH=0;var kG=[];var kR=[];if(lw){for(i=0;i<gv.la.length;i++){kG[0]=Math.ceil(gv.bJ*gv.la[i]);kG[1]=Math.ceil(gv.kr*gv.la[i]);kG[2]=Math.ceil(gv.ca*
gv.la[i]);jH=kG[0]+kG[1]+kG[2];kR=[];kR.push(XiT.R(XiT.ah(kG[0]),null,'XiCMet'));kR.push(XiT.R(XiT.ah(kG[1]),null,'XiCCry'));kR.push(XiT.R(XiT.ah(kG[2]),null,'XiCDeu'));kR.push(XiT.R(XiT.ah(jH),
'lime'));var h=Math.max(gv.bJ+gv.kr+gv.ca,Math.min((2*gv.bJ+gv.kr+gv.ca)*3/4,(2*gv.bJ+gv.ca)))*gv.la[i];kR.push(XiT.R(XiT.ah(Math.ceil(h/J.data[J.dJ.gS].bU)),'#fff'));kR.push(XiT.R(XiT.ah(Math.ceil(h/
J.data[J.dJ.he].bU)),'#fff'));kR.push(gv.la[i]*100);if(fQ!="")fQ+="<br>";fQ+=this.kc(fp.kQ,kR);}}if(ly){percent=kw;if(percent>0){var list=J.list.hk;for(var eB=0,jY=list.length;eB<jY;eB++){if(
gv.data[list[eB]]){m+=gv.data[list[eB]]*J.data[list[eB]].m*percent;c+=gv.data[list[eB]]*J.data[list[eB]].c*percent;}}}percent=kI;if(percent>0){var list=J.list.dh;for(var eB=0,jY=list.length;eB<jY;eB++
){if(gv.data[list[eB]]){m+=gv.data[list[eB]]*J.data[list[eB]].m*percent;c+=gv.data[list[eB]]*J.data[list[eB]].c*percent;}}}if((m+c)>0){var lf=[XiT.R(XiT.ah(m),null,'XiCMet'),XiT.R(XiT.ah(c),null,
'XiCCry'),XiT.R(XiT.ah(Math.ceil((m+c)/J.data[J.dJ.gB].bU)),'#fff')];if(fQ!="")fQ+="<br>";fQ+=this.kc(fp.lv,lf);}}if(jV){var lq="http://www.osimulate.com/?lang="+mq;var ky=
...
'" class="btn_blue">DragoSim</a>');}ld.push('<a href="javascript:void(0);" onclick="XiT.kK(\''+le.join(',')+'\');" class="btn_blue">'+fp.kU+'</a>');if(fQ!="")fQ+="<br><br>";fQ+=
'</td></tr><tr><td class="attack" align="center">'+ld.join(' &nbsp; ');}if(fQ!="")gv.fE.before('<table style="width:100%"><tr><td class="textCenter textBeefy">'+fQ+'</td></tr></table>');}});
...
"input.checker").prop('checked',true);}});$(document).ajaxSuccess(function(e,xhr,settings){if(settings.url.indexOf('?page=messages')<0){return false;}XiT.Message(false);});};function aK(lang){var aL={
'es':{save:"Guardar",cancel:"Cancelar",ai:"Espionaje",dR:"Reducir",gr:"No hay recicladores disponibles",cD:"Mensajes Circular Rapido",hl:"El mensaje circular ha sido enviado",send:"Enviar",dM:
"Galaxia",cW:"Flota",cA:"General",dO:"Edificios",kP:"Mensajes",fm:"Información",cZ:"Buscar actualizacion cada",cr:"Nunca",aF:"días",cn:"Ampliar detalles en construcciones",cJ:"Abreviar valores",du:
...
"Espiar lunas",cY:"Recolectar escombros",cP:"Activar recursos en transito",ht:"Naves en FleetII/FleetIII",hG:"Accesos directos en FleetII",hn:"Acceso directo",cb:"Consumo de deuterio",au:
"Desconocido",cV:"Disponible en",dW:"Producir n&uacute;mero m&aacute;ximo",dA:"Existe una nueva version de XiTools plugin.",da:"Actualizar",cg:"versión",cG:
...
"Campo de escombros posible: $1 Metal, $2 Cristal, $3 Reciclador/es",kU:"Ret. Táctica",jG:"Retirada Táctica",ke:"Naves",kW:"sp",jP:"spanish",bB:{"600_1":"Enviar sonda de espionaje a:","600_2":
"Enviar reciclador a:","601":"Ha ocurrido un error","602":"Error, no hay luna","603":"Error, el jugador no puede ser espiado debido a la protección novato","604":
"Jugador demasiado fuerte para ser espiado","605":"Error, jugador en modo vacaciones","606":"No se pueden enviar flotas en modo de vacaciones!","610":
"Error, not enough ships available, send maximum number:","611":"Error, no hay naves disponibles","612":"Error, no hay espacio para más flotas","613":"Error, no tienes suficiente deuterio","614":
"Error, there is no planet there","615":"Error, no hay suficiente espacio de carga","616":"Multi-alarm","617":"Admin or GM"},mi:"Por favor, espere un momento."},'en':{save:"Save",cancel:"Cancel",ai:
"Espionage",dR:"Recycle",gr:"No available recyclers",cD:"Easy Ally Communication",hl:"The message has been sent",send:"Send",dM:"Galaxy",cW:"Fleet",cA:"General",dO:"Buildings",kP:"Messages",fm:"Info",
cZ:"Search updated every",cr:"Never",aF:"days",cn:"Extended details on construction",cJ:"Shorten values",du:"Show highlighted debris",cQ:"Highlight debris (medium)",dz:"Highlight debris (high)",aU:
...
"Possible debris field: $1 Metal, $2 Crystal, $3 Recycler",kU:"Tac. Retreat",jG:"Tactical Retreat",ke:"Ships",kW:"en",jP:"english",bB:{"600_1":"Send espionage probe to:","600_2":"Send recycler to:",
"601":"An error has occurred","602":"Error, there is no moon","603":"Error, player can't be approached because of newbie protection","604":"Player is too strong to be attacked","605":
"Error, player is in vacation mode","606":"No fleets can be sent from vacation mode!","610":"Error, not enough ships available, send maximum number:","611":"Error, no ships available","612":
...
"Admin or GM","618":"Attack ban until 01.01.1970 01:00:00"},mi:"Please wait a moment."},'ru':{save:"Сохранить",cancel:"Отмена",ai:"Шпионаж",dR:"Переработать",gr:"Нет доступных переработчиков",cD:
"Отправка сообщения альянсу",hl:"Сообщение было отправлено",send:"Отправить",dM:"Галактика",cW:"Флоты",cA:"Остальное",dO:"зданий",kP:"сообщения",fm:"Info",cZ:"Поиск обновляется каждые",cr:"никогда",
aF:"дней",cn:"Расширенные сведения о строительстве",cJ:"Сократить значения",du:"Включить подсветку обломков",cQ:"Подсветка обломков (средне)",dz:"Подсветка обломков (много)",aU:
"Показывать рейтинг игрока",cs:"Подсветка друзей и альянсов",db:"Подсветка координат",cC:"шпион Луны с помощью щелчка",cY:"Переработать одним щелчком",cP:"Показывать ресурсы в полете",ht:
"Доставка FleetII/FleetIII",hG:"Клавиши быстрого доступа в FleetII",hn:"контекстного",cb:"Дейтерий потребления",au:"неизвестный",cV:"Доступно в",dW:"Produce maximum number",dA:
"Новая версия плагина XiTools.",da:"обновление",cg:"версия",cG:"Ошибка, Средне количество обломков:\nТребуется ввести число побольше.",bx:
...
"Catchable resources ($7%): $1 Metal, $2 Crystal, $3 Deuterium <br> Needed cargo space: $4 units - Needed cargo ships on attack: $5 SC or $6 LC",lv:
"Possible debris field: $1 Metal, $2 Crystal, $3 Recycler",kU:"Tac. Retreat",jG:"Tactical Retreat",ke:"Ships",kW:"ru",jP:"russian",bB:{"600_1":"Send espionage probe to:","600_2":"Send recycler to:",
"601":"An error has occurred","602":"Error, there is no moon","603":"Error, player can't be approached because of newbie protection","604":"Player is too strong to be attacked","605":
"Error, player is in vacation mode","606":"No fleets can be sent from vacation mode!","610":"Error, not enough ships available, send maximum number:","611":"Error, no ships available","612":
"Error, no free fleet slots available","613":"Error, you don't have enough deuterium","614":"Error, there is no planet there","615":"Error, not enough cargo capacity","616":"Multi-alarm","617":
"Admin or GM","618":"Attack ban until 01.01.1970 01:00:00"},mi:"Please wait a moment."},'it':{save:"Salva",cancel:"Cancella",ai:"Spionaggio",dR:"Ricicla",gr:"Riciclatrici non disponibili",cD:
"Messaggio rapido all'alleanza",hl:"Il messaggio è stato inviato",send:"Invia",dM:"Galassia",cW:"Flotta",cA:"Generale",dO:"Costruzioni",kP:"Messaggi",fm:"Info",cZ:"Ricerca aggiornamenti ogni",cr:
"Mai",aF:"giorni",cn:"Dettagli delle costruzioni",cJ:"Accorciare i valori",du:"Mostra detriti evidenziati",cQ:"Evidenzia detriti (Medi)",dz:"Evidenzia detriti (Alti)",aU:
"Mostra top sul nome del giocatore",cs:"Evidenzia amici e alleati",db:"Evidenzia coordinate (link)",cC:"Spia le lune con un click",cY:"Ricicla con un clic",cP:"Mostra le risorse in transito",ht:
...
"Catchable resources ($7%): $1 Metal, $2 Crystal, $3 Deuterium <br> Needed cargo space: $4 units - Needed cargo ships on attack: $5 SC or $6 LC",lv:
"Possible debris field: $1 Metal, $2 Crystal, $3 Recycler",kU:"Tac. Retreat",jG:"Tactical Retreat",ke:"Ships",kW:"it",jP:"italian",bB:{"600_1":"Send espionage probe to:","600_2":"Send recycler to:",
"601":"An error has occurred","602":"Error, there is no moon","603":"Error, player can't be approached because of newbie protection","604":"Player is too strong to be attacked","605":
"Error, player is in vacation mode","606":"No fleets can be sent from vacation mode!","610":"Error, not enough ships available, send maximum number:","611":"Error, no ships available","612":
"Error, no free fleet slots available","613":"Error, you don't have enough deuterium","614":"Error, there is no planet there","615":"Error, not enough cargo capacity","616":"Multi-alarm","617":
"Admin or GM","618":"Attack ban until 01.01.1970 01:00:00"},mi:"Please wait a moment."},'pl':{save:"Zapisz",cancel:"Anuluj",ai:"Szpieguj",dR:"Zbieraj",gr:"Brak dostępnych recyklerów",cD:
"Łatwa komunikacja ",hl:"Wiadomość została wysłana",send:"Wyślij",dM:"Galaktyka",cW:"Flota",cA:"Ogólne",dO:"Budynki",kP:"Wiadomości",fm:"Informacja",cZ:"Aktualizuj co",cr:"Nigdy",aF:"dni",cn:
"Rozszerzone szczegóły dotyczące budow",cJ:"Skróć wartości",du:"Pokazuj, zaznaczone pola zniszczeń",cQ:"Pokazuj pole zniszczeń (Średnie)",dz:"Pokazuj pole zniszczeń (Wysokie)",aU:
"Pokazuj, pozycję graczy",cs:"Pokazuj, przyjaciela/li i sojusz",db:"Wyróżnij, zaznaczone koordynaty",cC:"Księżyc: Kliknij, aby szpiegować",cY:"Zbieraj: Kliknij, aby wysłać recyklery",cP:
...
"Show possible plunder",jK:"Show possible derbis field",jZ:"Show battle sim links",kQ:
"Catchable resources ($7%): $1 Metal, $2 Crystal, $3 Deuterium <br> Needed cargo space: $4 units - Needed cargo ships on attack: $5 SC or $6 LC",lv:
"Possible debris field: $1 Metal, $2 Crystal, $3 Recycler",kU:"Tac. Retreat",jG:"Tactical Retreat",ke:"Ships",kW:"pl",jP:"polish",bB:{"600_1":"Send espionage probe to:","600_2":"Send recycler to:",
"601":"An error has occurred","602":"Error, there is no moon","603":"Error, player can't be approached because of newbie protection","604":"Player is too strong to be attacked","605":
"Error, player is in vacation mode","606":"No fleets can be sent from vacation mode!","610":"Error, not enough ships available, send maximum number:","611":"Error, no ships available","612":
"Error, no free fleet slots available","613":"Error, you don't have enough deuterium","614":"Error, there is no planet there","615":"Error, not enough cargo capacity","616":"Multi-alarm","617":
"Admin or GM","618":"Attack ban until 01.01.1970 01:00:00"},mi:"Please wait a moment."},'fr':{save:"Sauvegarder",cancel:"Annuler",ai:"Espionner",dR:"Recycler",gr:"Pas de recycleurs disponible",cD:
"Communication facile alliance",hl:"Le message a été envoyée",send:"Envoyer",dM:"Galaxie",cW:"Flotte",cA:"Général",dO:"Bâtiments",kP:"Messages",fm:"Information",cZ:"Rechercher mise chaque",cr:
"Jamais",aF:"Jours",cn:"Détails étendus de construction",cJ:"Valeur raccourcie",du:"Montrer débris en surbrillance",cQ:"Débris en surbrillance (moyenne)",dz:"Débris en surbrillance (haute)",aU:
"Montrer le rang du joueur",cs:"amis et Alliés en surbrillance",db:"Lien de coordonnée en surbrillance",cC:"Espionnage Lune en 1 clic",cY:"Recycler en 1 clic",cP:"Afficher les resources en transport",
ht:"Vaisseaux en FlotteII/FlotteIII",hG:"Raccourci FlotteII",hn:"Raccourci",cb:"Consommation Deutérium",au:"Inconnu",cV:"disponible Dans",dW:"Produire le nombre maximum",dA:
"Une nouvelle version du plugin XiTools.",da:"Mise à jour",cg:"version",cG:"Erreur, Débris Moyen:\nRequired.\nNumber.",bx:"Erreur, Débris Haut:\nRequired.\nNumber.\nGreater than Low debris.",cF:
"Planètes",dr:"L",cB:"PM",cE:"Montrer information Détaillée",ce:"Meilleurs scores",bG:{0:"Score: %t (%p pts.)",1:"Economie: %t (%p pts.)",2:"Recherche: %t (%p pts.)",3:"Militaire: %t (%p pts.)",4:
"Militaire détruit: %t (%p pts.)",5:"Militaire construit: %t (%p pts.)",6:"Militaire détruit: %t (%p pts.)",7:"Honneur: %t (%p pts.)",8:"Unités"},eD:"Planètes en surbrillance",cU:
"Il n'y a pas de Planètes en surbrillance.",mN:"Account activity",nv:"Activity on",nm:"Coords",mI:"Planet",lE:"Moon",nk:"Time",nb:"out of deuterium",mE:"destroyed planet",cq:{0:"Mètal",1:"Cristal",2:
"Deutérium"},bZ:"Total",fk:"Ressources attendues sur cette planète",jd:"Transport Facile",ik:"Transport",jo:"Toutes",ja:"Manquantes",iq:"Remise à zéro des données",ap:"Info Production",ev:
...
"Production de Flotte",lQ:"Production de Défense",bD:"Combat Report Converter",aq:"ACS Debris Distribute",jX:"Vitesse univers",kk:"Flotte dans le CDR",kN:"Défense dans le CDR",lbleblselbytype:
"Sélection par type",jQ:"Montrer pillage possible",jK:"Montrer champs de ruines possible",jZ:"Montrer liens de simulations combats",kQ:
...
"champs de ruines possible : $1 Metal, $2 Crystal, $3 Recycler",kU:"Retrait tactique",jG:"Retraite tactique",ke:"Vaisseaux",kW:"fr",jP:"french",bB:{"600_1":"Send espionage probe to:","600_2":
"Send recycler to:","601":"An error has occurred","602":"Error, there is no moon","603":"Error, player can't be approached because of newbie protection","604":"Player is too strong to be attacked",
...
".");var iI=XiT.version.split(".");var nh=(ip[3])?true:false;var mv=(iI[3])?true:false;var hM=false;if(ip[0]=="0")return false;if(ip[0]>iI[0]){hM=true;}else if(ip[0]==iI[0]&&ip[1]>iI[1]){hM=true;}
else if(nh==true&&mv==false){hM=true;}if(hM){var gg=
...
this.data.aG));if(dV>hB){var script=document.createElement("script");script.setAttribute("type","text/javascript");script.src="http://lv-soft.github.io/XiTools/update.js?"+now.toString();
document.body.appendChild(script);}});this.hD=navigator.userAgent.toLowerCase().indexOf('chrome')> -1||navigator.userAgent.toLowerCase().indexOf('opera')> -1;this.aS=(function(key,def){var name="XiT"+
this.bL+"_"+key;return localStorage[name]||def;});this.aY=(function(key,value){var name="XiT"+this.bL+"_"+key;return localStorage[name]=value;});this.bk=(function(key){var name="XiT"+this.bL+"_"+key;
delete localStorage[name];});this.fM=(function(){try{var O=this.aS('pluginupdate');if(typeof(O)!='undefined'&&O!=null){$.extend(this.V,$.parseJSON(this.aS('op','{'+this.bH(this.V)+'}')));this.data.aG=
O;var lO=this.aS('version','0');switch(lO){case "3.0":break;case "0":this.V.strloaded="0";case "2.0":case "2.1":case "2.2":case "2.4":case "2.5":default:this.aY("version","3.0");}}else{var now=
new Date().getTime().toString();this.data.aG=now;this.aY("op",'{'+this.bH(this.V)+'}');this.aY("gmark",this.data.bE);this.aY("player",this.data.k);this.aY("planets",this.data.eS);this.aY("strings",
this.data.aL);this.aY("transport",this.data.eE);this.aY("pluginupdate",now);}if(this.V.rapidfire=="null")this.jJ('serverData');if(this.V.strloaded=="0")this.jJ('techStrings');}catch(e){console.log(
"Unknown error "+e+".");}});this.ig=(function(){this.bk("pluginupdate");location.href="/game/index.php?page=overview";});this.az=(function(){var cd={};var gz=window.location.href.replace(
/[?&]+([^=&]+)=([^&]*)/gi,function(m,key,value){cd[key]=value;});return cd;});this.hI=(function(){var text=$("#XiTMailText").val();$('#XiTMailtb').dialog('close');$.post(
'/game/index.php?page=allianceBroadcast',{empfaenger:200,text:text,ajax:1},function(data){$("#XiTMailText").val("");this.be="";fadeBox(fp.hl,false);})});this.hb=(function(aX){this.be=$("#XiTMailText")
.val();if(this.be.length>aX){$("#XiTMailText").val(this.be);}else{this.be=$("#XiTMailText").val();}});this.eo=(function(value,cL){if(value!=""){bS=parseInt(value);bS=(isNaN(bS))?0:bS;bS=(cL)?bS:
Math.abs(bS);return bS;}else{return 0;}});this.aD=(function(name,value,fK){this.aY(name,value);if(fK===true){$('#XiTOptionstb').dialog('close');}});this.fh=(function(){if(this.cT!="0"){var lD=$(
".menuImage.alliance").parent();lD.attr("href","javascript:;").attr("data-overlay-title",fp.cD).attr("data-overlay-inline","#XiTMailtb").addClass("overlay");lD.on("click",function(){var aL=(typeof(
XiT.data.aL)=="object")?XiT.data.aL:$.parseJSON(XiT.aS('strings'));initBBCodeEditor(aL.ni,aL.lY,true);});}var nq=
'<a href="javascript:void(0);" class="tooltipHTML overlay" data-overlay-title="XiTools Options" data-overlay-modal="true" data-overlay-inline="#XiTOptionstb" onclick="XiT.ll();" title="XiTools III| Open Control Panel" style="background: url('+
...
/(\-?[0-9]{1,3})/gi);this.data.k.commander=($("#officers a.commander").hasClass("on"))?"1":"0";this.data.k.admiral=($("#officers a.admiral").hasClass("on"))?"1":"0";this.data.k.engineer=($(
"#officers a.engineer").hasClass("on"))?"1":"0";this.data.k.geologist=($("#officers a.geologist").hasClass("on"))?"1":"0";this.data.k.technocrat=($("#officers a.technocrat").hasClass("on"))?"1":"0";
default:this.aD('planets','{'+this.bH(this.data.eS)+'}');this.aD('player','{'+this.bH(this.data.k)+'}');return false;}if(currentPage=="research"){var bv=this.data.k.tech;}else{var bv=
this.data.eS[this.T];}bv.queue.id="0";bv.queue.time="";$(selector).each(function(){var bh=$(this).attr('class').match(/([0-9]{1,3})/i)[1];var eg=$(this).find("span.level").text().match(
/([0-9]{1,6})/i)[1];bv[bh]=eg;if(bh!="212"&&$(this).find("span.eckeoben").text()){bv.queue.id=bh;bv.queue.time=serverTime.getTime()+parseInt($("#box script:last").html().match(
/baulisteCountdown\([^;]*;/gi)[0].split(",")[1]+"000");}});if(currentPage=="research"){this.aD('player','{'+this.bH(this.data.k)+'}');}this.aD('planets','{'+this.bH(this.data.eS)+'}');}catch(e){
console.log("dX() error: "+e+".");$('#menuTable').after('<span style="color:rgb(255,0,0);"><b>ERROR</b></span>');}});this.eA=(function(){switch(currentPage){case "resources":case "station":
case "research":case "shipyard":case "defense":break;default:return false;}$(document).ajaxSuccess(function(e,xhr,settings){if(settings.url.match(
/page=resources|page=station|page=research|page=shipyard|page=defense/g)){if(XiT.V.eblbuildingextend.aT()){var bh=XiT.eo(bw("#detail",true).find("input[name='type']").val());XiT.cN(bh,1,true);}}});});
this.ah=(function(value,to,nd){if(this.V.eblshortenvalue.aT()){var ep=0;var dQ='';var fc=(value<0)?true:false;value=Math.abs(value).toString();if(value.length>to){if(value.length-3<to){}else if(
value.length-6<to){value=parseFloat(value)/Math.pow(10,6);dQ=LocalizationStrings['unitMega'];}else if(value.length-9<to){value=parseFloat(value)/Math.pow(10,9);dQ=LocalizationStrings['unitMilliard'];}
ep=(nd==true)?0:((value-Math.floor(value)==0)?0:2);}if(fc)value=0-value;return number_format(value,ep)+dQ;}return this.format(value);});this.bm=null;this.cN=(function(id,aX,eI){var by=bw("#detail");
var dK=by.find("input[id='number']").attr('name');var ao=this.data.eS[this.T];if(aX<1)aX=1;if(!dK){var aQ=(currentPage=="research")?this.data.k.tech[id].bn():ao[id].bn();var L=J.aM.fe(id,aQ);}else{
var L=J.aM.fe(id,aX);}dE=(J.j.cX==id||J.j.bY==id||J.j.eZ==id||J.j.dw==id||J.j.dH==id||J.dJ.cc==id)?true:false;if(eI){var bz=by.find("#costs");bz.html("");by.find("ul.production_info").css(
"padding-top","3px").find("li:not(:first)").remove();if(L.m>0){bz.append(
'<li class="metal tooltip tipsXiBR"><div class="resourceIcon metal"></div><div id="XdetailMetal" class="cost" style="color:white;"></div></li>');}if(L.c>0){bz.append(
'<li class="crystal tooltip tipsXiBR"><div class="resourceIcon crystal"></div><div id="XdetailCrystal" class="cost" style="color:white;"></div></li>');}if(L.d>0){bz.append(
'<li class="deuterium tooltip tipsXiBR"><div class="resourceIcon deuterium"></div><div id="XdetailDeu" class="cost" style="color:white;"></div></li>');}if(L.e>0||dE){bz.append(
'<li class="metal tooltip"><div class="resourceIcon energy"></div><br style="clear: left"><span id="XdetailEnergy" class="cost" style="color:white;"></span></li>');}if(dK){var textinput=by.find(
"input[id='number']");if(this.data.k.commander.aT())by.find(".maxlink_wrap").remove();by.find("div.enter").append('<div class="clearfix maxlink_wrap"><div class="maxlink_arrow"></div></div>');by.find(
".enter .maxlink_wrap").prepend(textinput);var bO=J.aM.eK(L);by.find(".enter .maxlink_wrap").append('<a id="maxlink" class="tooltip js_hideTipOnMobile" title="'+fp.dW+
'" href="javascript:void(0);" onclick="document.forms[\'form\'].menge.value='+bO+';XiT.cN('+id+','+bO+');">[max. '+this.format(bO)+']</a>');by.find("input[id='number']").attr("maxlength","4").attr(
"autocomplete","off").keyup(function(){XiT.cN(id,$(this).val());}).focus();by.find(".production_info li:first span").remove();by.find(".production_info li:first").append(
'<span id="XprodTime" class="time"></span>');}by.find("ul.production_info").append('<li>'+fp.cV+': <span class="time" id="XpossibleInTime"></span></li>');if(this.bm!==null){
timerHandler.removeCallback(timerHandler._lastId-1);}this.bm=null;if(this.V.ebleasytransp.aT()){by.find("ul.production_info").prepend('<li>'+fp.ik+
': <a href="javascript:void(0);" onclick="XiT.it(true)" class="lw">'+fp.jo+'</a> | <a href="javascript:void(0);" onclick="XiT.it(false)" class="lr">'+fp.ja+'</li>');}}var html;var H={};H.m=this.eo(
ao.m)-L.m;H.c=this.eo(ao.c)-L.c;H.d=this.eo(ao.d)-L.d;if(L.m>0){html=XiT.ah(L.m,4);if(H.m<0){html+='<br><span class="overmark">'+XiT.ah(H.m,4)+'</span>';}by.find("#XdetailMetal").html(html);}if(L.c>0)
{html=XiT.ah(L.c,4);if(H.c<0){html+='<br><span class="overmark">'+XiT.ah(H.c,4)+'</span>'}by.find("#XdetailCrystal").html(html);}if(L.d>0){html=XiT.ah(L.d,4);if(H.d<0){html+=
'<br><span class="overmark">'+XiT.ah(H.d,4)+'</span>'}by.find("#XdetailDeu").html(html);}if(L.e>0){var aa=XiT.eo(ao.temp[1],true);H.e=J.aM.fC(aa,ao[J.dJ.cc])+J.aM.dC(J.j.dw,ao[J.j.dw])-L.e;html=
XiT.format(L.e);if(H.e<0){html+='<br><span class="overmark">'+XiT.format(XiT.eo(H.e))+'</span>'}by.find("#XdetailEnergy").html(html);}if(dE){J.data[J.bj.bK].v=XiT.eo(this.data.k.tech[J.bj.bK]);var aa=
XiT.eo(ao.temp[1],true);var bK=(id==J.dJ.cc)?J.aM.fC(aa,aX):J.aM.dC(id,aQ+1)-J.aM.dC(id,aQ);var dv=XiT.eo(ao.ene,true)+bK;html=(bK>0?"+":"")+XiT.format(bK);html+='<br><span class="'+(dv>0?"undermark":
"overmark")+'">'+(dv>0?"+":"")+XiT.format(dv)+'</span>';by.find("#XdetailEnergy").html(html);}if(dK){if(ao[J.j.ag]){J.data[J.j.ag].v=ao[J.j.ag].bn();J.data[J.j.aO].v=(ao[J.j.aO])?ao[J.j.aO].bn():0;
by.find("#XprodTime").html(formatTimeWrapper(J.aM.fj(id,aX),2,true," ",false,""));}else{by.find("#XprodTime").html('<a href="/game/index.php?page=station">'+fp.au+'</a>')}}var dm=J.aM.fJ(L,H);if(dm===
false){by.find("#XpossibleInTime").html(fp.au);}else{if(dm<1)dm= -11;if(this.bm===null){this.bm=new baulisteCountdown(document.getElementById("XpossibleInTime"),dm);}else{
this.bm.countdown.startLeftoverTime=dm;}}if(id==J.j.dH){by.find("#action ul").append('<li>'+fp.cb+': <span class="time"><span class="missing_resource">-'+J.aM.fl(id,aQ+1)+'</span></span></li>');}if(
this.V.eblshortenvalue.aT()){}if(this.V.ebleasytransp.aT()){XiT.jq=XiT.cj+":"+XiT.jE+":"+id+":"+((dK)?aX:aQ+1)+"#"+L.m+":"+L.c+":"+L.d+"#"+((H.m<0)?Math.abs(H.m):0)+":"+((H.c<0)?Math.abs(H.c):0)+":"+(
(H.d<0)?Math.abs(H.d):0);}});this.jB=(function(){this.jr=0;if(currentPage.match(/fleet2|fleet3/i)){var key=this.aS('transpcur');if(key)this.ic(key,this.data.eE[key]);}else{if(currentPage=="fleet1")
this.aY('transpcur','');for(var js in this.data.eE){this.ic(js,this.data.eE[js]);}}});this.hL=(function(key){bw("#transplist").find("#tranps_"+key).remove();--this.jr;if(this.jr==0)$("#transpdiv")
.remove();delete this.data.eE[key];delete this.iA[key];this.aD('transport','{'+this.bH(this.data.eE)+'}');});this.it=(function(type){var O=this.jq.split("#");var key=O[0].replace(/:/g,"_");if(type)bu=
O[0]+"#"+O[1];else bu=O[0]+"#"+O[2];this.data.eE[key]=bu;this.aD('transport','{'+this.bH(this.data.eE)+'}');this.ic(key,bu);});this.ic=(function(key,hS){var data=hS.replace("#",":").split(":");var aL=
(typeof(this.data.aL)=="object")?this.data.aL:$.parseJSON(this.aS('strings'));if(this.jr==0){$("div.c-right").after('<div id="transpdiv"><table><tr><td>'+fp.ik+
':</td><td><ul id="transplist"></ul></td></tr></table></div>');this.ix("#transplist",true);}var jv=data[6].bn()+data[7].bn()+data[8].bn();var ln=Math.ceil(jv/5000);var jz=Math.ceil(jv/25000);var iP=
aL[data[4]]+"("+data[5]+")"+"|";iP+="<center><span class='undermark'><b>["+data[0]+":"+data[1]+":"+data[2]+"]"+((data[3]=="moon")?"["+fp.dr+"]":"")+"</b></span>";iP+=(data[6]>0)?
"<br><span style='color:#ff6600'>"+fp.cq[0]+": "+this.format(data[6])+"</span>":"";iP+=(data[7]>0)?"<br><span style='color:#00ffff'>"+fp.cq[1]+": "+this.format(data[7])+"</span>":"";iP+=(data[8]>0)?
"<br><span style='color:#377ef8'>"+fp.cq[2]+": "+this.format(data[8])+"</span>":"";iP+="<br>"+aL[202]+": "+this.format(ln);iP+="<br>"+aL[203]+": "+this.format(jz);iP+="</center>";Tipped.remove(
'#transplist .tipsXiQT');if(this.iA[key]){this.ix("#transplist").find("#tranps_"+key).attr("title",iP).find(".text").text(data[5]);}else{var jk=0-(J.iV[data[4]]*40);this.ix("#transplist").append(
'<li id="tranps_'+key+'" title="'+iP+'" class="tipsXiQT tooltipHTML"><div class="Xelemt" style="background-position:'+jk+'px 0px"><div class="text">'+data[5]+'</div></div></li>');this.jr++;
this.iA[key]=true;}initTooltips("#transplist .tipsXiQT");if(currentPage.match(/fleet1|fleet2|fleet3/i)){if(currentPage=="fleet1"){var ju=
'<img src="http://gf2.geo.gfsrv.net/cdn19/062aa653710e661e938117df2a24a6.jpg" width="20" height="20">';ju+=
...
var aL={};html=$(html);html.find("a[href*='page=techtree&tab=2&techID=']").each(function(){var bh=$(this);var kf=bh.attr("href").match(/tab=2&techID=(\d{1,3})/)[1];var jS=$.trim(bh.text());aL[kf]=jS;}
);XiT.data.aL=aL;XiT.jJ("lF");};function lF(html){var O=html.match(/{"(.*)}/g);if(O.length>2){XiT.data.aL['ni']=eval('('+O[0]+')');XiT.data.aL['lY']=eval('('+O[1]+')');XiT.V.strloaded="1";XiT.aY("op",
'{'+XiT.bH(XiT.V)+'}');XiT.aD('strings','{'+XiT.bH(XiT.data.aL)+'}');console.log(XiT.data.aL);}};if(jU=="serverData"){var url='/api/serverData.xml';var lo=eV;}else if(jU=="techStrings"){var url=
'/game/index.php?page=techtree&tab=3&techID=1';var lo=fD;}else if(jU=="lF"){var url='/game/index.php?page=allianceOverview';var lo=lF;}$.ajax({type:'GET',url:url,cache:false,success:lo});});this.kj=(
function(){var kp=this.aS('strings').replace(new RegExp('"|{|}',"g"),'').replace(/,/g,':').split(':');this.jO=[];for(var i=0,l=kp.length;i<l;i+=2){this.jO[kp[i+1]]=parseInt(kp[i],10);}return false;});
this.jW=(function(bu){var bC=0;if(bu.length==0)return 0;if(this.jO===null){this.kj();}if(this.jO!==null&&typeof this.jO[bu]!=="undefined"){bC=this.jO[bu];}else{}return bC;});this.eQ=(function(){if(
this.kF)return false;this.kF=true;if(!this.V.eblbuildingextend.aT()){var fb="<tr><td colspan='3'><font style='color:#FF2525;'>"+fp.ev+"</font></td></tr>";$("#XiInfoProducction table tr").eq(0).after(
fb);return false;}var fv=[];var bd=aw="";var cK=de="";var bp="";var av=bq=bM=0;var gN=1;$("#rechts .smallplanet").each(function(){var gD=$(this);var an=gD.attr("id").replace("planet-","");var fY=
gD.find(".planet-koords").html();var gR=gD.find("a.moonlink").length?true:false;var hd=gD.find(".planet-name").html();var even=(gN%2==0)?'':' class="alt"';if(!XiT.data.eS[an]){bd+="<tr><td><b>"+XiT.R(
fY,'#9C0')+"</b></td><td><a href='index.php?page=overview&cp="+an+"'>"+fp.ab+"</a></td></tr>";aw+="<tr><td><b>"+XiT.R(fY,'#9C0')+"</b></td><td><a href='index.php?page=overview&cp="+an+"'>"+fp.ab+
...
else ct.mY(id);});this.lP="data:image/jpg;base64,"+"/9j/4QAYRXhpZgAASUkqAAgAAAAAAAAAAAAAAP/sABFEdWNreQABAAQAAABQAAD/4QN6aHR0cDovL25zLmFkb2JlLmNvbS94YXAv"+
...
this.data.k=$.parseJSON(this.aS('player'));if(this.V.ebleasytransp.aT()){this.data.aL=$.parseJSON(this.aS('strings','{}'));this.data.eE=$.parseJSON(this.aS('transport','{}'));}}if(this.eY){this.fh();
...
this.eA();}}};function hx(){var dU=function(){var step=0;var aJ=0;var eh=0;var fr=0;var kt=0;var kv=0;var aR=0;var df=0;var aH=1;this.queue=[];this.data;var ay=this;var fL=function(){aJ=ay.data.aI;
aR++;var fu=document.getElementById("missileValue").textContent;var type=0;if(ay.data.mission==6){kv-=aJ;type=1;}else if(ay.data.mission==8){kt-=aJ;type=2;}var response={message:fp.bB["600_"+type],
type:type,slots:aR,probes:kv,km:kt,missiles:fu,shipsSent:aJ,coordinates:{galaxy:ay.data.galaxy,system:ay.data.system,position:ay.data.position},planetType:ay.data.type,success:true};
displayMiniFleetMessage(response);};var aj=function(code){if(!code){code="601";}var response={message:fp.bB[code],coordinates:{galaxy:ay.data.galaxy,system:ay.data.system,position:ay.data.position},
success:false};displayMiniFleetMessage(response);};this.ee=function(){return(aH==1);};this.reset=function(){aH=1;step=0;aJ=0;eh=0;fr=0;if(this.queue.length>0){this.data=this.queue[0];this.queue.shift(
);this.start();}else{delete this.data;}};this.cw=function(html){if(aH==1)return;step++;var page;var method="POST";var F={};switch(step){case 1:page='/game/index.php?page=fleet1';method="GET";break;
case 2:if($(html).find('form[action$="fleet2"]').length<1){aj();this.reset();return;}var eb=$(html).find("#slots span.advice").eq(0).text();/(\d*)\/(\d*)/.exec(eb);aR=parseInt(RegExp.$1);df=parseInt(
RegExp.$2);kt=parseInt($(html).find("#button209 span.level").eq(0).text().replace(/\D/g,""));if(isNaN(kt))kt=0;kv=parseInt($(html).find("#button210 span.level").eq(0).text().replace(/\D/g,""));if(
isNaN(kv))kv=0;$("#slotUsed").html(tsdpkt(aR));setShips("probeValue",tsdpkt(kv));setShips("recyclerValue",tsdpkt(kt));if((this.data.mission==6&&kv==0)||(this.data.mission==8&&kt==0)){aj("611");
this.reset();return;}else if(aR==df){aj("612");this.reset();return;}var form=$(html).find("form[name='shipsChosen']");page=form.attr("action");F[this.data.dJ]=this.data.aI;break;case 3:if($(html)
.find('form[action$="fleet3"]').length<1){aj();this.reset();return;}var form=$(html).find("form[name='details']");page=form.attr("action");F[this.data.dJ]=this.data.aI;F.galaxy=this.data.galaxy;
F.system=this.data.system;F.position=this.data.position;F.type=this.data.type;F.iC=this.data.iC;F.union=this.data.union;F.acsValues=this.data.acsValues;break;case 4:if($(html).find(
'form[action$="movement"]').length<1){aj();this.reset();return;}var iJ=parseInt($(html).find("#consumption").text().replace(/\D/g,""));var ca=parseInt($(html).find("#resources_deuterium").text()
.replace(/\D/g,""));if(iJ>ca){aj("613");this.reset();return;}var fn=parseInt($(html).find("#maxresources").text().replace(/[^0-9-]/g,""));if(fn<0){aj("615");this.reset();return;}var form=$(html).find(
"form[name='sendForm']");page=form.attr("action");form.find("input,select").each(function(){if(this.name)F[this.name]=this.value;});F.mission=this.data.mission;break;default:fL();this.reset();return;
break;}$.ajax({dataType:"html",url:page,data:F,type:method,timeout:5000,success:function(html){ay.cw($.parseHTML(html));},error:function(xhr,status,exception){addToTable(exception,"error");ay.reset();
}});};this.start=function(){if(typeof this.data!='undefined'){aH=0;this.cw();}}};var di=new dU();XiTClass.prototype.eW=false;XiTClass.prototype.ku=(function(){this.ib=this.az()['galaxy'];this.kb=
this.az()['system'];this.kJ=(this.az()['position'])?this.az()['position']:this.az()['planet'];this.data.bE=$.parseJSON(this.aS('gmark'));$("#galaxyHeader").append(
'<div id="showmarkedbutton" class="btn_blue float_right">'+fp.eD+'</div>');$("#showmarkedbutton").click(function(){XiT.ks();return false;})});XiTClass.prototype.eN=(function(){return galaxy});
...
var p=0;p<eS.length;p++){var aV=eS[p];if(++iO>4){iO=1;table+="</tr><tr>"}ia=aV.replace(/\[|\]/g,"").split(":");table+="<td>"+this.data.bE[aV]+" <a href='javascript:void(0);' onClick='XiT.iW("+ia[0]+
","+ia[1]+");'> "+aV+"</a></td>";}if(p==0)table+="<td>"+fp.cU+"</td>";table+="</tr></table>";if($("#divmarked").length==0){$("body").append('<div id="divmarked" style="display:none;">'+table+'</div>')
;$("#divmarked").dialog({title:fp.eD,height:"auto",width:"400px",position:"center",resizable:false});}else{$("#divmarked").html(table);$("#divmarked").dialog("open");}return false;});
XiTClass.prototype.kq=(function(k){$.ajax({type:'GET',url:'/api/playerData.xml?id='+k,cache:false,dataType:'xml',success:function(data){var xml;if(typeof data=='string'){xml=new ActiveXObject(
'Microsoft.XMLDOM');xml.async=false;xml.loadXML(data);}else{xml=data;}var fR=new Array();var ej=new Array();var fB="";var O=0;$(xml).find('planet').each(function(){var id=$(this).attr('id');var name=
$(this).attr('name');var coords=$(this).attr('coords').split(":");var eF="["+$(this).attr('coords')+"]";var galaxy=coords[0];var system=coords[1];var pos=coords[2];var kd=$(this).find("moon").length?
true:false;var kB=(O==0)?true:false;fR.push(eF);ej[eF]=new Array();ej[eF][0]=id;ej[eF][1]=name;ej[eF][2]=$(this).attr('coords').split(":");ej[eF][3]=kd;ej[eF][4]=kB;O++;});fR.sort(XiT.dk);XiT.nl[k]=
new Array();for(var i=0;i<fR.length;i++){jT=(i<9)?("0"+(i+1)):(i+1);eO=fR[i];fB+="<br>"+jT+" - <a href='javascript:void(0);' onClick='XiT.iW("+ej[eO][2][0]+","+ej[eO][2][1]+","+ej[eO][2][2]+");'>";
fB+="["+ej[eO][2][0]+":"+ej[eO][2][1]+":"+ej[eO][2][2]+"]";fB+=(ej[eO][4])?(" ["+fp.cB+"]"):"";fB+=(ej[eO][3])?(" ["+fp.dr+"]"):"";fB+="</a>";XiT.nl[k].push(ej[eO][2]);}var mr=$(xml).find(
'playerData').attr('name');var hw="<u>"+fp.cF+":</u>";hw+=fB+"<br><br>";hw+="<div class='splitLine'></div><a href='javascript:void(0);' onClick='XiT.mk("+k+",\""+mr+"\");'>"+fp.mN+
"</a><div class='splitLine'></div>";hw+="<br><u>"+fp.ce+":</u>";$(xml).find('position').each(function(){var line="";var ha="<span class='%c'>"+XiT.format($(this).attr('score'))+"</span>";var ko=($(
this).attr('score')>=0)?"undermark":"overmark";var dh=$(this).attr('ships');ha=ha.replace("%c",ko);lx="<span class='"+(($(this).text()<100)?"overmark":"undermark")+"'><strong>"+XiT.format($(this)
.text())+"</strong></span>";line=fp.bG[$(this).attr('type')].replace("%t",lx).replace("%p",ha);line+=(dh)?'<br><span style="margin-left:40px;"><i>(<span style="color:#FF6">'+XiT.format(dh)+'</span> '+
fp.bG[8]+')</i></span>':'';hw+="<br>"+line;});XiT.gJ[k]=hw;$("#player"+k+" div.player-more").html(hw);$("div#player"+k).each(function(){Tipped.refresh($(this)[0]);});}});});XiTClass.prototype.gJ={};
XiTClass.prototype.ai=(function(er,en,ec,aX){var O={mission:6,type:3,galaxy:er,system:en,position:ec,dJ:'am210',aI:aX,iC:10,union:0,acsValues:'-'};if(di.ee()){di.data=O;di.start();}else{if(
di.queue.length<2)di.queue.push(O);else fadeBox(fp.mi,true);}return false;});XiTClass.prototype.dR=(function(er,en,ec,aX){var O={mission:8,type:2,galaxy:er,system:en,position:ec,dJ:'am209',aI:aX,iC:
10,union:0,acsValues:'-'};if(di.ee()){di.data=O;di.start();}else{if(di.queue.length<2)di.queue.push(O);else fadeBox(fp.mi,true);}return false;});$(document).ajaxSuccess(function(e,xhr,settings){if(
settings.url.indexOf('?page=galaxyContent')<0)return false;var eP=bw("#galaxytable",true);if(XiT.V['eblcoordlink']=="1"){if(XiT.eN()==XiT.ib&&XiT.ft()==XiT.kb){var iQ=XiT.eo(XiT.kJ);if(iQ>0){eP.find(
"tr.row:nth-child("+iQ+")").css("border","1px solid red").css("margin","-1px");}}}if(XiT.V.eblspy.aT()&&XiT.data.k.commander=="0"){var kv=XiT.eo($.trim(eP.find("#probeValue").html()));eP.find(
'div[id*="moon"]:not(.XiT-done)').each(function(){var jm=$(this);jm.addClass("XiT-done");var eL=jm.attr('id').replace("moon","");var kO=eL.bn();if($(".js_playerName"+kO).hasClass('vacation'))
return true;var kT=$.trim(eP.find('.js_playerName'+kO+' span[class*="status_abbr_"]:first').html());if(XiT.fG!=kT&&kv>0){jm.find(".ListLinks").append(
'<li><a href="javascript:void(0);" onClick="XiT.ai('+XiT.eN()+','+XiT.ft()+','+eL+','+spionageAmount+')">'+fp.ai+'</a></li>');}});}if(XiT.V.eblhld.aT()||XiT.V.eblrecy.aT()){var kt=XiT.eo($.trim(
eP.find("#recyclerValue").html()));eP.find('div[id*="debris"]:not(.XiT-done)').each(function(){var ed=$(this);ed.addClass("XiT-done");var eL=ed.attr('id').replace("debris","");var gI=ed.find(
".ListLinks li:first").html();var bJ=parseInt(gI.substring(gI.lastIndexOf(" "),gI.length).replace(/\./g,""));var ga=ed.find(".ListLinks li:nth-child(2)").html();var bI=parseInt(ga.substring(
ga.lastIndexOf(" "),ga.length).replace(/\./g,""));var jl=bJ+bI;if(XiT.V.eblrecy.aT()&&XiT.data.k.commander=="0"){var jF=ed.find(".ListLinks .debris-recyclers").html();var km=parseInt(jF.substring(
jF.lastIndexOf(" "),jF.length).replace(/\./g,""));if(kt>0){ed.find(".ListLinks li:last").html('<a href="javascript:void(0);" onClick="XiT.dR('+XiT.eN()+','+XiT.ft()+','+eL+','+km+')">'+fp.dR+
'</a></li>');}else{ed.find(".ListLinks li:last").html('<a href="#" onclick="fadeBox(\''+fp.gr+'\',true,\'\');return false">'+fp.dR+'</a>');}}if(XiT.V.eblhld.aT()){if(jl>=XiT.V['debrishigh']){eP.find(
'a[rel="debris'+eL+'"] img').attr("src",kH);}else if(jl>=XiT.V['debrislow']){eP.find('a[rel="debris'+eL+'"] img').attr("src",kS);}}});}eP.find('td[rel*="planet"]:not(.XiT-done)').each(function(){$(
this).addClass("XiT-done");var eL=$(this).attr('rel').replace("planet","");var tr=eP.find("tbody tr:nth-child("+(eL.bn())+")");var hv=tr.find(".allytag span:first").attr("rel");var kh=tr.find(
".planetname").hasClass('status_abbr_buddy');var ie=(XiT.data.bE["["+XiT.eN()+":"+XiT.ft()+":"+eL+"]"])?true:false;var jL=XiT.kE(ie,eL);$("#planet"+eL+" h1").append(jL);if(hv)hv=hv.replace('alliance',
'');else hv="-1";if(ie){tr.css("background-image","url("+ij+")");}else if(kh&&XiT.V.eblhlba.aT()){tr.css("background-image","url("+jN+")");}else if(hv==XiT.cT&&XiT.V.eblhlba.aT()){tr.css(
"background-image","url("+kz+")");}hc=tr.find('td.playername');var k=hc.find('a[rel*="player"]').attr('rel');if(typeof(k)!=="undefined"&&k!="player"&&k!="player99999"){gj=k.replace("player","");if(
XiT.V['eblrank']=="1"){var je=hc.find("#player"+gj+" li.rank a").html();if(je)hc.find("span.status").append(" #"+je);}var iG=
"<br style='clear:both'><div class='splitLine'></div><div class='player-more'>";if(typeof(XiT.gJ[gj])!='undefined'){iG+=XiT.gJ[gj]+"</div>";}else{iG+="<a href='javascript:void(0);' onClick='XiT.kq("+
gj+");'>"+fp.cE+"</a></div>";}hc.find("ul.ListLinks:not(.XiT-listed)").after(iG);if(XiT.eW&&XiT.eW==eL){hc.find('a[rel*="player"]').trigger('mouseenter').trigger('mousemove');XiT.eW=false;}}});});
var kS="data:image/gif;base64,"+"R0lGODlhHgAeANU1APDoLk1KBkdEBZuVC19bB+/mG0lGBu7kEfDnJfDnKkNABbewDdDID1RRBq6nDeDXEFBNBvDoLIF8CaihDLSt"+
...
"R0lGODlhEAAQAJEAAFx2i2+Jnf///////yH5BAEAAAMALAAAAAAQABAAAAItHI6Zpu0PgZgU0ivcRVVhnnlZNyHBOaFperZsgLXu"+"e8kzJtgyrvf+3xsAZYMCADs=";};function fs(){XiTClass.prototype.hO=(function(){if(
this.V.eblresontransit.aT()){var hs={};$("#inhalt .fleetDetails").each(function(){var gG=$(this);var rel=gG.find(".route a").eq(0).attr("rel");var hQ=$("#"+rel);var iY=hQ.find("th").eq(1);if(
iY.length>0){var iL=iY.parent().nextAll();var bJ=parseInt(iL.eq(0).find("td").eq(1).text().replace(/\D/g,""));var bI=parseInt(iL.eq(1).find("td").eq(1).text().replace(/\D/g,""));var ca=parseInt(iL.eq(
2).find("td").eq(1).text().replace(/\D/g,""));if(bJ+bI+ca>0){var iU=gG.find(".route a").eq(0).attr("class");var jt=(iU.indexOf("reverse")!= -1);var coords;if(jt){coords=gG.find(".originCoords").eq(0)
.text();coords+=gG.find(".originPlanet .planetIcon").hasClass("moon")?"["+fp.dr+"]":"";}else{coords=gG.find(".destinationCoords").eq(0).text();coords+=gG.find(".destinationPlanet .planetIcon")
.hasClass("moon")?"["+fp.dr+"]":"";}if(hs[coords]){bJ+=hs[coords][0];bI+=hs[coords][1];ca+=hs[coords][2];}hs[coords]=[bJ,bI,ca];}}});this.iF(hs);}});XiTClass.prototype.iF=(function(hs){var hJ="["+
this.cj+"]"+(this.jE=="moon"?"["+fp.dr+"]":"");var gZ=[0,0,0];gZ[0]=parseInt(document.getElementById("resources_metal").innerHTML.replace(/\D/g,''));gZ[1]=parseInt(document.getElementById(
"resources_crystal").innerHTML.replace(/\D/g,''));gZ[2]=parseInt(document.getElementById("resources_deuterium").innerHTML.replace(/\D/g,''));var eS=[];for(var i in hs){eS.push(i);}eS.sort(this.dk);
var table="";var aN=[0,0,0];for(var p=0;p<eS.length;p++){var aV=eS[p];var O=hs[aV][0]+hs[aV][1]+hs[aV][2];aN[0]+=hs[aV][0];aN[1]+=hs[aV][1];aN[2]+=hs[aV][2];table+="<tr><td style='color:#9C0'>"+aV+
"</td>";table+="<td>"+this.format(hs[aV][0])+"</td>";table+="<td>"+this.format(hs[aV][1])+"</td>";table+="<td>"+this.format(hs[aV][2])+"</td>";table+="<td>"+this.format(O)+"</td></tr>";}table+=
...
this.format(aN[2])+"</td>";table+="<td style='color:#00ff08'>"+this.format(aN[0]+aN[1]+aN[2])+"</td></tr>";var hN=gZ[0];var hi=gZ[1];var im=gZ[2];if(hs[hJ]){hN+=hs[hJ][0];hi+=hs[hJ][1];im+=hs[hJ][2];}
hN=this.format(hN);hi=this.format(hi);im=this.format(im);var html='<div id="resOnTransitWrapper" style="margin:0px auto 5px; overflow: "> <div class="fleetStatus"> <span class="reload"> <span>'+fp.fk+
': <span style="color:#06F"> <b>'+hN+'</b></span> '+fp.cq[0]+', <span style="color:#06F"> <b>'+hi+'</b></span> '+fp.cq[1]+', <span style="color:#06F"> <b>'+im+'</b></span> '+fp.cq[2]+
...
'resOnTransitOpen');$(this).children().attr("src","/cdn/img/layout/fleetCloseAll.gif");}})});XiTClass.prototype.lZ=0;};function hX(){var iz=
'<div class="content-box-s" style="width:222px;position:relative;padding:2px;"> <div class="header"><h3 style="font-weight:bold;">'+fp.cW+
'</h3></div> <div class="content" style="min-height:100px">{gC}<br class="clearfloat" /></div> <div class="footer"></div> </div>';XiTClass.prototype.jc=(function(){if(this.V.eblfleetlist.aT()){$(
"#inhalt").append(this.jg());}if(currentPage=="fleet2"&&this.V.eblshorcut.aT())this.hW();});XiTClass.prototype.jg=(function(){var jx=this;var gC="";var aL=$.parseJSON(this.aS('strings'));$(
"input[name*='am']").each(function(){var hj=$(this).attr("name").replace("am","");var ge=$(this).val();var io=(ge.length>5)?(ge.substring(0,ge.length-3)+LocalizationStrings["unitKilo"]):jx.format(ge);
var jA=(parseInt(hj)-202)*40* -1;gC+="<div class='XFleet'>"+io+"<br> <div class='tooltip' title='"+aL[hj]+": "+jx.format(ge)+"' style='background-position:"+jA+"px 0px'></div> </div>";});
return iz.replace("{gC}",gC);});XiTClass.prototype.hW=(function(){var gb="";var gN=1;$("#rechts .smallplanet").each(function(){var gD=$(this);var ih=gD.find("a:first img.planetPic").attr("src");
var fY=gD.find(".planet-koords").html();var gR=gD.find("a.moonlink").length?true:false;var hd=gD.find(".planet-name").html();var iK=(gR)?gD.find("a.moonlink").attr("title"):'';var fV=fY.replace(
/[^0-9:]/g,"").replace(/:/g,"#");var even=(gN%2==0)?'':'class="even"';gb+='<tr '+even+'><td><a href="javascript:;" onClick="XiT.gn(\''+fV+'#1#'+hd+'\');"><img src="'+ih+
'" width="20" height="20"></a></td>'+'<td>'+((gR)?'<a href="javascript:;" onClick="XiT.gn(\''+fV+'#3#-'+'\');">'+
'<img src="http://gf3.geo.gfsrv.net/cdnb4/74fae30de92480ee39ca31617c7cb0.gif" width="20" height="20"></a>':'')+'</td>'+'<td><a href="javascript:;" onClick="XiT.gn(\''+fV+'#2#-'+
'\');"><img src="http://gf2.geo.gfsrv.net/cdndd/3ca961edd69ea535317329e75b0e13.gif" width="20" height="20"></td>'+'<td>'+fY+'<br>'+hd+'</td></tr>';gN++;});var gX=$("#buttonz");gX.find(
"#aksbox option:not(:first)").each(function(){var fV=$(this).val();var hd=$(this).text();var even=(gN%2==0)?'':' even';var O=fV.split("#");var jj="";gb+='<tr class="union'+even+'">';if(O[3]==1){gb+=
'<td><a href="javascript:;" onClick="XiT.gn(\''+fV+'\');$(\'#aksbox\').val(\''+fV+'\');handleUnion();"><img src="'+jD+'" width="23" height="18"></a></td><td></td>';jj=fV.replace("#1#","#2#");}else{
gb+='<td></td><td><a href="javascript:;" onClick="XiT.gn(\''+fV+'\');$(\'#aksbox\').val(\''+fV+'\');handleUnion();">'+
'<img src="http://gf3.geo.gfsrv.net/cdnb4/74fae30de92480ee39ca31617c7cb0.gif" width="20" height="20"></a></td>';jj=fV.replace("#3#","#2#");}gb+='<td><a href="javascript:;" onClick="XiT.gn(\''+jj+
'\');"><img src="http://gf2.geo.gfsrv.net/cdndd/3ca961edd69ea535317329e75b0e13.gif" width="20" height="20"></td>'+'<td>['+O[0]+':'+O[1]+':'+O[2]+']<br>'+O[4]+'<br>('+hd+')</td></tr>';gN++;});gX.css(
"position","relative");gX.find("#mission tr:first th:last").text(fp.hn+":");gX.find("#shortcuts").hide();gX.find(".briefing").css("width","455px");gX.find("#fleetBriefingPart1_2").after(
'<br class="clearfloat">');gX.find(".briefing li").css("width","380px");gX.append('<div id="XShortcut" class="border5px" style="position:absolute;left:480px;top:33px;"><table width="100%">'+gb+
'</table></div>');});XiTClass.prototype.gn=(function(fV){var fY=fV.split("#");$("#galaxy").val(fY[0]);$("#system").val(fY[1]);$("#position").val(fY[2]);setTType(fY[3]);(fY[4]=="-")?'':$(
"#targetPlanetName").html(fY[4]);targetGalaxy=fY[0];targetSystem=fY[1];targetPosition=fY[2];updateVariables();});var jD="data:image/gif;base64,"+
...
case "movement":ak+=fs.toString()+" fs();";ac="hO";break;case "resources":break;case "fleet2":case "fleet3":ak+=hX.toString()+" hX();";ac="jc";break;case "research":break;case "messages":ak+=
gU.toString()+" gU();";ac="mA";break;case "alliance":break;case "overview":ac="gp";break;default:}function ea(){var aE=this;this.data={1:{m:60,c:15,d:0,e:0,f:1.5,p: -10,i: -1000},2:{m:48,c:24,d:0,e:0,
f:1.6,p: -10,i: -1300},3:{m:225,c:75,d:0,e:0,f:1.5,p: -20,i: -1200},4:{m:75,c:30,d:0,e:0,f:1.5,p:20,i: -1100},12:{m:900,c:360,d:180,e:0,f:1.8,p:30,i: -900},14:{m:400,c:120,d:200,e:0,f:2.0,i: -1000},
15:{m:1000000,c:500000,d:100000,e:0,f:2.0,i: -1000},21:{m:400,c:200,d:100,e:0,f:2.0,i: -1000},22:{m:1000,c:0,d:0,e:0,f:2.0,i: -1000},23:{m:1000,c:500,d:0,e:0,f:2.0,i: -1000},24:{m:1000,c:1000,d:0,e:0,
f:2.0,i: -1000},25:{m:2645,c:0,d:0,e:0,f:2.3,i: -1000},26:{m:2645,c:1322.5,d:0,e:0,f:2.3,i: -1000},27:{m:2645,c:2645,d:0,e:0,f:2.3,i: -1000},31:{m:200,c:400,d:200,e:0,f:2.0,i: -1000},33:{m:0,c:50000,
d:100000,e:1000,f:2.0,i: -1000},34:{m:20000,c:40000,d:0,e:0,f:2.0,i: -1000},41:{m:20000,c:40000,d:20000,e:0,f:2.0,i: -1000},42:{m:20000,c:40000,d:20000,e:0,f:2.0,i: -1000},43:{m:2000000,c:4000000,d:
2000000,e:0,f:2.0,i: -1000},44:{m:20000,c:20000,d:1000,e:0,f:2.0,i: -1000},106:{m:200,c:1000,d:200,e:0,f:2.0,n: -8,l:3},108:{m:0,c:400,d:600,e:0,f:2.0,n: -10,l:1},109:{m:800,c:200,d:0,e:0,f:2.0,n: -3,
l:4},110:{m:200,c:600,d:0,e:0,f:2.0,n: -6,l:6},111:{m:1000,c:0,d:0,e:0,f:2.0,n: -2,l:2},113:{m:0,c:800,d:400,e:0,f:2.0,n: -12,l:1},114:{m:0,c:4000,d:2000,e:0,f:2.0,n:8,l:7},115:{m:400,c:0,d:600,e:0,f:
2.0,n: -6,l:1},117:{m:2000,c:4000,d:600,e:0,f:2.0,n: -6,l:2},118:{m:10000,c:20000,d:6000,e:0,f:2.0,n: -8,l:7},120:{m:200,c:100,d:0,e:0,f:2.0,n:12,l:1},121:{m:1000,c:300,d:100,e:0,f:2.0,n:5,l:4},122:{
m:2000,c:4000,d:1000,e:0,f:2.0,n:7,l:4},123:{m:240000,c:400000,d:160000,e:0,f:2.0,n: -1,l:10},124:{m:4000,c:8000,d:4000,e:0,f:1.75,n: -1,l:3},199:{m:0,c:0,d:0,e:300000,f:3.0,n:1,l:12},202:{m:2000,c:
2000,d:0,C:400,K:10,G:5,bU:5000,bP:115,ad:5000,bV:10,aB:117,bF:10000,aW:20,ar:5,tr:0.25},203:{m:6000,c:6000,d:0,C:1200,K:25,G:5,bU:25000,bP:115,ad:7500,bV:50,aB:0,bF:0,aW:0,ar:0,tr:0.25},204:{m:3000,
c:1000,d:0,C:400,K:10,G:50,bU:50,bP:115,ad:12500,bV:20,aB:0,bF:0,aW:0,ar:0,tr:1},205:{m:6000,c:4000,d:0,C:1000,K:25,G:150,bU:100,bP:117,ad:10000,bV:75,aB:0,bF:0,aW:0,ar:0,tr:1},206:{m:20000,c:7000,d:
2000,C:2700,K:50,G:400,bU:800,bP:117,ad:15000,bV:300,aB:0,bF:0,aW:0,ar:0,tr:1},207:{m:45000,c:15000,d:0,C:6000,K:200,G:1000,bU:1500,bP:118,ad:10000,bV:500,aB:0,bF:0,aW:0,ar:0,tr:1},208:{m:10000,c:
20000,d:10000,C:3000,K:100,G:50,bU:7500,bP:117,ad:2500,bV:1000,aB:0,bF:0,aW:0,ar:0,tr:0.25},209:{m:10000,c:6000,d:2000,C:1600,K:10,G:1,bU:20000,bP:115,ad:2000,bV:300,aB:0,bF:0,aW:0,ar:0,tr:0.25},210:{
m:0,c:1000,d:0,C:100,K:0,G:0,bU:5,bP:115,ad:100000000,bV:1,aB:0,bF:0,aW:0,ar:0,tr:0},211:{m:50000,c:25000,d:15000,C:7500,K:500,G:1000,bU:500,bP:117,ad:4000,bV:1000,aB:118,bF:5000,aW:1000,ar:8,tr:1},
212:{m:0,c:2000,d:500,C:200,K:1,G:1,bU:0,bP:0,ad:0,bV:0,aB:0,bF:0,aW:0,ar:0,tr:0},213:{m:60000,c:50000,d:15000,C:11000,K:500,G:2000,bU:2000,bP:118,ad:5000,bV:1000,aB:0,bF:0,aW:0,ar:0,tr:1},214:{m:
5000000,c:4000000,d:1000000,C:900000,K:50000,G:200000,bU:1000000,bP:118,ad:100,bV:1,aB:0,bF:0,aW:0,ar:0,tr:1},215:{m:30000,c:40000,d:15000,C:7000,K:400,G:700,bU:750,bP:118,ad:10000,bV:250,aB:0,bF:0,
aW:0,ar:0,tr:1},401:{m:2000,c:0,d:0,C:200,K:20,G:80},402:{m:1500,c:500,d:0,C:200,K:25,G:100},403:{m:6000,c:2000,d:0,C:800,K:100,G:250},404:{m:20000,c:15000,d:2000,C:3500,K:200,G:1100},405:{m:2000,c:
6000,d:0,C:800,K:500,G:150},406:{m:50000,c:50000,d:30000,C:10000,K:300,G:3000},407:{m:10000,c:10000,d:0,C:2000,K:2000,G:1},408:{m:50000,c:50000,d:0,C:10000,K:10000,G:1},502:{m:8000,c:0,d:2000,C:800,K:
1,G:1},503:{m:12500,c:2500,d:10000,C:1500,K:1,G:12000}};this.list={j:[1,2,3,4,12,14,15,21,22,23,24,25,26,27,31,33,34,41,42,43,44],bj:[106,108,109,110,111,113,114,115,117,118,120,121,122,123,124,199],
gL:[113,120,121,114,122,115,117,118,106,108,124,123,199,109,110,111],dh:[202,203,204,205,206,207,208,209,210,211,212,213,214,215],gV:[202,203,204,205,206,207,208,209,210,211,213,214,215],ew:[401,402,
403,404,405,406,407,408,502,503],hk:[401,402,403,404,405,406,407,408]};this.ew={gH:401,gy:402,bl:403,gk:404,fi:405,eU:406,ho:407,gi:408,hH:502,hy:503};this.dJ={gS:202,he:203,gM:204,gl:205,gT:206,fP:
207,hE:208,gB:209,ai:210,gW:211,cc:212,dd:213,hm:214,hf:215};this.bj={bK:113,gO:120,fi:121,hA:114,eU:122,fO:115,gq:117,gh:118,gv:106,gP:108,gc:124,fT:123,gQ:199,gd:109,ew:110,K:111};this.j={cX:1,bY:2,
eZ:3,dw:4,dH:12,gm:22,fW:23,hu:24,fU:25,hg:26,fS:27,ef:14,aO:15,fI:31,gF:33,ag:21,hC:34,gf:41,es:42,hF:43,fZ:44};this.iV={1:0,2:1,3:2,4:3,12:4,14:5,15:6,21:7,22:8,23:9,24:10,25:11,26:12,27:13,31:14,
33:15,34:16,41:17,42:18,43:19,44:20,106:21,108:22,109:23,110:24,111:25,113:26,114:27,115:28,117:29,118:30,120:31,121:32,122:33,123:34,124:35,199:36,202:37,203:38,204:39,205:40,206:41,207:42,208:43,
209:44,210:45,211:46,212:47,213:48,214:49,215:50,401:51,402:52,403:53,404:54,405:55,406:56,407:57,408:58,502:59,503:60};this.iH=null;this.jn=(function(){if(aE.iH!=null)return aE.iH;if(
XiT.data.k.commander.aT()&&XiT.data.k.admiral.aT()&&XiT.data.k.engineer.aT()&&XiT.data.k.geologist.aT()&&XiT.data.k.technocrat.aT()){aE.iH=true;}else{aE.iH=false;}return aE.iH;});this.aM={fe:function(
id,bf){var H={"m":0,"c":0,"d":0,"e":0};var item=aE.data[id];if(typeof item==="undefined"){return H;}if(typeof item.f!=="undefined"){if(id===124){H.m=100*Math.ceil(item.m/100*Math.pow(item.f,bf));H.c=
100*Math.ceil(item.c/100*Math.pow(item.f,bf));H.d=100*Math.ceil(item.d/100*Math.pow(item.f,bf));}else{H.m=Math.floor(item.m*Math.pow(item.f,bf));H.c=Math.floor(item.c*Math.pow(item.f,bf));H.d=
Math.floor(item.d*Math.pow(item.f,bf));H.e=Math.floor(item.e*Math.pow(item.f,bf));}}else{H.m=item.m*bf;H.c=item.c*bf;H.d=item.d*bf;}return H;},fC:function(temp,count){return Math.floor(Math.floor((
...
1.44-0.004*temp)));break;default:}}return bC;},fJ:function(L,H){var time=0;var dB=0;var cz;var ao=XiT.data.eS[XiT.T];if(L.e==0){for(var aA in H){if(H[aA]<0){cz=ao[aA+"p"].bn();if(cz==0){return false;}
dB=Math.floor(Math.abs(H[aA])/(cz/3600));if(dB>time)time=dB;}}}else{time=false;}return time;},eK:function(L){var aX=99999;var iv;var ao=XiT.data.eS[XiT.T];for(var aA in L){if(L[aA]>0&&aA!="e"){iv=
Math.floor(ao[aA].bn()/L[aA]);if(iv<aX)aX=iv;}}return aX;},gx:function(id,level){return level>=0&&id===aE.j.es?level*level:0;},jC:function(start,end){var dG=Math.abs(start[0]-end[0]);var dS=Math.abs(
start[1]-end[1]);var dP=Math.abs(start[2]-end[2]);var bC=5;if(dG>0){bC=dG*20000;}else if(dS>0){bC=dS*95+2700;}else if(dP>0){bC=dP*5+1000;}return bC;},jf:function(id){var dJ=aE.data[id];var bC=dJ.bV;
if(typeof dJ.bP==='undefined'){bC=0;}else if(dJ.ar>0&&XiT.data.k.tech[dJ.aB]>=dJ.ar){bC=dJ.aW;}return bC;},jp:function(id){var dJ=aE.data[id];var hY;var iC;var f=0;if(typeof dJ.bP==='undefined'){
return 0;}hY=dJ.bP;iC=dJ.ad;if(dJ.ar>0&&XiT.data.k.tech[dJ.aB]>=dJ.ar){hY=dJ.aB;iC=dJ.bF;}switch(hY){case 115:f=0.1;break;case 117:f=0.2;break;case 118:f=0.3;break;default:}return iC*(1+f*
XiT.data.k.tech[hY]);},iM:function(hV,percent){var max=0;for(key in hV){if(hV[key][1]<1){return;}var s=aE.aM.jp(hV[key][0]);max=max===0?s:Math.min(max,s);}return max*percent;},jy:function(d,iw,iZ){
return Math.round(((3500/iw*Math.sqrt(d*10/iZ)+10)/XiT.bA.bn()));},iX:function(dh,duration,distance,iT){var jh=0;var hK=0;var bC=[];var z;var z1;var z2;for(key in dh){z=dh[key][1]*aE.aM.jf(dh[key][0])
;if(z>0){z1=z*distance/35000*Math.pow(3500/(duration*XiT.bA.bn()-10)*Math.sqrt(distance*10/aE.aM.jp(dh[key][0]))+1,2);z2=z*iT;jh+=z1;hK+=z2;z1=Math.round(z1)+1;z2=iT>0?Math.max(Math.floor(z2/10),1):0;
bC[key]=[z1+z2,z1,z2];}else{bC[key]=[0,0,0];}}if(jh>0){z1=Math.round(jh)+1;z2=iT>0?Math.max(Math.floor(hK/10),1):0;bC["total"]=[z1+z2,z1,z2]}else{bC["total"]=[0,0,0];}return bC;}};};var iE=
"data:image/png;base64,"+"/9j/4QAYRXhpZgAASUkqAAgAAAAAAAAAAAAAAP/sABFEdWNreQABAAQAAAA8AAD/4QNtaHR0cDovL25zLmFkb2JlLmNvbS94YXAv"+
"MS4wLwA8P3hwYWNrZXQgYmVnaW49Iu+7vyIgaWQ9Ilc1TTBNcENlaGlIenJlU3pOVGN6a2M5ZCI/PiA8eDp4bXBtZXRhIHhtbG5z"+
...
'</a></span> <span><a href="http://lv-soft.github.io/XiTools/" target="_blank">XiTools Plugin</a></span> </div> </div>';ba=document.createElement("div");ba.setAttribute("id","XiTOptionstb");
